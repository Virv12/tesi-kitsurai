...
a requirement often overlooked in major existing open-source solutions such as Cassandra~\cite{Cassandra} and MongoDB~\cite{RT-Mongo}.
...
a distributed database suitable for real-time performance guarantees.
...
Additional experimentation with HTTP/3 and QUIC~\cite{QUIC2021} could further improve latency and connection management.
...
or X-Stor’s Request Units (RUs)~\cite{Lei2024}.

Several additional features could be built atop the existing system as middleware layers:
\begin{itemize}
    \item A fine-grained access control mechanism based on standard authentication and authorization frameworks such as OAuth 2.0 or JWT~\cite{RFC7519};
