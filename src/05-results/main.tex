...
We configured the machines for either a uniform key distribution, where each machine's throughput parameter was set to the same sufficiently high value,
or with a proportional key distribution, where each machine's throughput parameter was still set sufficiently high but proportional to its self-stress results.
...
with sufficient throughput to saturate the test case, $N=1$, $R=1$, and $W=1$.
This configuration distributes requests across peers according to each machine's throughput proportion.
...
In our setup the slowest machine is $A$, which achieved \qty{4.8}{\kilo\rps},
yielding an expected overall throughput of \qty{14.4}{\kilo\rps}.
During testing the system reached \qty{13.2}{\kilo\rps} (\qty{92}{\percent} of expected),
...
In the proportional case, as long as the machine throughput are configured correctly no single machine should become a bottleneck
and the overall throughput should equal the sum of individual throughputs.
In our setup, this corresponds to \qty{20.6}{\kilo\rps},
while the test measured \qty{17.0}{\kilo\rps} or \qty{83}{\percent} of the expected value.
...
To test replication we used a table with sufficient throughput, $N=2$, $R=2$, and $W=2$.
...
The expected overall throughput is \qty{7.2}{\kilo\rps}, and testing confirms the system achieves \qty{7.2}{\kilo\rps}.
...
In our setup $H$ is faster than $M$ and $A$ combined, giving an expected overall throughput of \qty{9.8}{\kilo\rps}.
The system achieved \qty{9.4}{\kilo\rps} or \qty{96}{\percent} of the expected value.
...
\begin{table}
    ...
    \begin{tabular}{lSSS}
        \toprule
        \multirow{2}{*}{\raisebox{-0.5ex}{Test Case}} & \multicolumn{3}{c}{Throughput (\unit{\kilo\rps})} \\
        \cmidrule{2-4}
...
With a proportional throughput configuration matching the measured rates in the self stress,
a table $T_T$ was created with $N=3$, $R=3$, $W=3$ and a throughput of \qty{200}{\rps}.
To represent external interference, another table $T_O$ was created with $N=2$, $R=2$, $W=2$
and throughput such that it would saturate the machines configured throughput.
...
The system successfully isolated the target table $T_T$ from the outside noise, maintaining the allocated throughput of \qty{200}{\rps}.
Reported in Table~\ref{tab:results-isolation} are the measured latencies when the target table received less than \qty{200}{\rps},
