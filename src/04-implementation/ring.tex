...
\[
    \mathbb{E}[w_i] = \sum_{k \in K_i} p_k \cdot T
\]
where $K_i$ is the set of keys assigned to node $i$, $p_k$ is the
probability that key $k$ is requested and $T$ is the total throughput of the
table, in requests per second.

An extreme request distribution prevents scaling: if some key $k$ has $p_k = 1$
and is placed on node $i$, then $\mathbb[w_i] = T$, which would require
$s_i \geq T$, i.e., node $i$ must handle the entire table's workload.
...
$\mathbb{E}[w_i] = \frac{|K_i|}{|K|} \cdot T$, so we can rewrite the second
requirement as
\[
    \frac{|K_i|}{|K|} \leq \frac{s_i}{T}.
\]

Since we don't want to waste resources, we strengthen the requirement and
enforce that $\frac{|K_i|}{|K|} = \frac{s_i}{T}$.
Summing over all nodes from the same AZ $z$, we get $\sum_{i :
\mathrm{az}(i) = z} \frac{|K_i|}{|K|} = \sum_{i : \mathrm{az}(i) = z}
\frac{s_i}{T}$, since each key is assigned to at most one node from the same
AZ, we have $\sum_{i : \mathrm{az}(i) = z} |K_i| \leq |K|$, hence
\[
    \sum_{i : \mathrm{az}(i) = z} s_i \leq T
\]
and $s_i \leq T$ for each node $i$.
...
\cdot |K|$, and so $S = T \cdot N$.

To solve this problem we will use the \emph{replication ring}.
This structure will require that the values $S$, $T$ and all $s_i$ be integers
...
is $\frac{N}{S} = \frac{1}{T}$, so the expected number of keys assigned to the
position is $|K| \cdot \frac{1}{T}$.
...
to node $i$ is $|K_i| = |K| \cdot \frac{s_i}{T}$, thus satisfying the second
requirement.
...
\]
An AZ $z$ is \emph{saturated} if $S_z = T$.
...
Since each residue class contributes exactly $T$ positions and $S_z \le T$,
...
Since $S_z \le T$, the run cannot complete a full circle and wrap around to the
...
\]
Since $S_{az(0)} = s_0 = 4 = T$, the AZ of node $0$ is saturated; all other AZs are non-saturated.
...
$T$ elements, and after $\left\lfloor \frac{p}{N} \right\rfloor$ elements of
its own class.
Hence, the index of $p$ in the \emph{Position order} is
\[
    o_p = \Bigl\lfloor \frac{p}{N} \Bigr\rfloor + (p \bmod N) \cdot T.
\]
...
\]
Since the workload on node $i$ is $w_i = |K_i| \cdot \frac{T}{|K|}$, we have
\[
    \mathbb{E}[w_i] = s_i
    \quad
    \sigma_{w_i} = \frac{T}{\sqrt{|K|}} \cdot \sqrt{p_i \cdot (1 - p_i)}.
\]
