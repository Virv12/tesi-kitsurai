...
\begin{enumerate}
    ...
    \item $\mathbb{E}[w_i] \leq s_i$ for each node $i$, where $w_i$ is the
        workload on node $i$ and $s_i$ is the bandwidth allocated by that node
        for the table, both measured in requests per second.
\end{enumerate}

The expected workload on the $i$-th node is $\mathbb{E}[w_i] = \sum_{k \in K_i}
p_k \cdot B$ where $K_i$ is the set of keys assigned to node $i$, $p_k$ is the
...
that $p_k = 1$ we get that $\mathbb{E}[w_i] = B \leq s_i$, requiring that node
$i$ is able to handle the entire table workload.
...
requirement as $\frac{|K_i|}{|K|} \leq \frac{s_i}{B}$.
...
This procedure runs in $O(S)$ time and uses $O(S)$ space.
...
\[
    ...
    S = B \cdot N = 12,
\]
and five nodes, each in a distinct AZ ($az(i) = i$), with allocated bandwidth
\[
...
and select nodes $r_p, r_{p+1}, \dots, r_{p+N-1}$ (with wrap-around), e.g., if
$p = 2$ then the key is assigned to nodes $2$, $0$, and $1$
...
allocated bandwidth, such that the expected workload on each node is within its
capacity.
...
table's bandwidth to accommodate potential slowdowns.
