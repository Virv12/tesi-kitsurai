...
based on consistent hashing from~\cite{ConsistentHashing}, gossip-based membership,
and Merkle-tree anti-entropy for repair. % TODO: Related Work: "merkle-tree anti-entropy" mi sembra abbia bisogno di una spiegazione rapida, soprattutto quando scrivete che ce l'ha anche \Kitsurai ?!?
...
As with Dynamo's sematic reconciliation, where the user is asked to pick the latest version on conflicts,
Cassandra’s append-only write design results in more expensive reads, but enables significantly higher write throughput.
...
% TODO: Cucinotta: Likely cite Bigtable here (verificate se hanno qualche nota/commento nel paper su consistent/stable multi-tenant performance etc...)
% Other distributed data stores, like Google's \textit{Bigtable}~\cite{Bigtable}, solve another issue?
% Cioè fanno la stessa cosa che fanno Mongo o Cassandra

Our work builds upon these foundations by maintaining a peer-uniform, leaderless architecture with gossip and Merkle-tree anti-entropy. % TODO: Stessa cosa di sopra.
...
Differently from Dynamo's reconciliation the system is not capable of any type of reconciliation
and is completely the  
