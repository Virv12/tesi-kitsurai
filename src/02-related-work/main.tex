...
built on Karger et al.'s \textit{consistent hashing}~\cite{ConsistentHashing}, gossip-based membership, and synchronization based on a Merkle-tree.
Since maintaining strong consistency is costly in the presence of network partitions and failures, Dynamo adopts \emph{eventual consistency}.
...
or handed to the user, who is returned multiple versions of a record to choose from (semantic reconciliation).
...
Amazon's DynamoDB~\cite{Dynamo2022, DynamoWhitepaper} later introduced the notion of per-table throughput quotas,
...
This is used both to enforce tenant-level request limits and by the system scheduler to migrate or scale hot partitions dynamically.
...
It carries over DynamoDB's per-table throughput reservations, but introduces a creation-time admission control,
...
For the sake of simplicity, we do not address dynamic partition splitting, and we focus instead on the simpler case of uniform key access.
Moreover, to further simplify the design,
our system removes consistency guarantees entirely, delegating the resolution of conflicting values to the user, basically reusing the original Dynamo's semantic reconciliation approach: a read operation simply returns all the values stored on different replicas.

Gupta et al.~\cite{FailureDetection} demonstrate that traditional heartbeat-based failure detectors do not scale,
