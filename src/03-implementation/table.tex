...
allowing the nodes to advertise these only.
...
\begin{enumerate}
    ...
    \item\label{step:prepare} The router iterates over the nodes in the
        ...
        for the new table, allocating up to $b$ on each availability zone.
        Each node can allocate as much bandwidth as it has available, up to the
...
delete the table, allowing the bandwidth to be freed.
...
the table as locally \emph{deleted}, freeing the allocated bandwidth.
