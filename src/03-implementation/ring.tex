...
|K_i| = \frac{S}{b}$.
Since each key is assigned to exactly \texttt{n} nodes, we have $\sum |K_i| = n
...
The key is then assigned to the \texttt{n} nodes immediately following the
...
def build_ring(table):
  ...
  # Assume nodes are already sorted as described above
