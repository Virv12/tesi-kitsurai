...
\begin{itemize}
    \item \textbf{No replica reconciliation.}
        ...
        until a later client write updates them (this is a limitation of the
        current prototype).
...
solid-state drives.
Indeed, rotational HDDs exhibit seek time and rotational latency that cause
high variance and long-tail stalls, invalidating the constant-throughput
abstraction required by the allocator; therefore, SSDs are required to provide
sufficiently predictable performance for the guaranteed rates.
...
The client provides four parameters: $B$, $N$, $R$, and $W$.
The system allocates sufficient bandwidth across the nodes to satisfy the
requested configuration, creates the table, and returns the generated
\texttt{id} of the new table.
If the requested parameters cannot be satisfied, for instance due to
insufficient available bandwidth or an insufficient number of Availability
Zones to meet the desired replication factor, the operation fails and an error
is returned.
...
nodes; otherwise, it returns an error and includes any values successfully
collected.
