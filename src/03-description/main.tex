...
A \Kitsurai system operates on a \emph{cluster} composed of a static (see
...
\begin{itemize}
    ...
    \item \texttt{read\_concern} or $R$: the minimum number of nodes that must
        successfully respond to a read request (see Section
        \ref{subsec:consistency}).
...
\Kitsurai adopts a conservative, request-per-second based admission policy.
