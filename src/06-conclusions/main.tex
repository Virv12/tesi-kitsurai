...
The \Kitsurai project was developed with the goal of creating a distributed key-value store
...
a requirement often overlooked in major existing open-source solutions such as Cassandra~\cite{Cassandra} and MongoDB~\cite{RTMongo}.
...
\begin{itemize}
    ...
    \item achieved performance gains due to scaling across multiple configurations, remaining within a small variance of the expected rate.
\end{itemize}
...
as can be seen in Dynamo~\cite{Dynamo2007} and its derivatives.

Similarly, table bandwidth parameters are currently static and cannot be reconfigured at runtime.
A natural extension would involve dynamic reallocation of table bandwidth and key-space distribution,
...
particularly when tables have unbalanced bandwidth or storage requirements.
Enhancing the current bandwidth-limiting model to support separate limits for read and write operations
...
% TODO: Ultimo par. del relwork: sembra out of context, almeno cosi';
%        secondo me potrebbe venir rigirato come un'aggiunta al Future Work (in fondo, assieme alle Conclusioni, dopo le conclusioni),
%        cioe' il fatto che in futuro si potrebbe provare a usare priorita' real-time (come fatto in RT-MongoDB),
%        o persino SCHED_DEADLINE qualora il sistema riuscisse a tracciare quali sono le deadline entro cui le richieste vanno completate.
%        Pero', dato che qui si parla di accesso al disco, puo' aver senso accennare a ioprio(_get/set()), cioe' lo scheduler del disco, prima di, o assieme a, quello della CPU (e lo scheduling dei pkt in rete?)
